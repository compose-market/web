...
const FALLBACK_INSTALL_PATH = "/install-desktop";
...
type ConnectMode = "desktop-first" | "web-first";
...
function parseQueryParams(): {
  ...
  return { deviceId, redirect, agentWallet, mode: "desktop-first" };
}
...
export default function ConnectDesktopPage() {
  ...
  const handleAuthorize = useCallback(async () => {
    ...
    if (mode === "desktop-first" && !deviceId) {
      setError("Desktop-first flow requires a device_id");
      return;
    }
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/desktop/link-token`, {
        ...
        body: JSON.stringify({
          deviceId: mode === "desktop-first" ? deviceId : undefined,
  ...
  const subtitle = mode === "desktop-first"
    ? "Authorize this already-running desktop app."
    : "Create a desktop install intent and open the local app via deep link.";

  if (error && mode === "desktop-first" && !deviceId) {
    return (
          ...
          <CardContent className="space-y-4">
            <div className="bg-sidebar-accent rounded-sm p-4 border border-sidebar-border">
              ...
              <p className="text-xs text-muted-foreground">
                You are authorizing a desktop runtime linked to your Compose account.
              </p>
            </div>
